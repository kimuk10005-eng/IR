...
def create_window_with_label(signal, changes, start, base, floor_type, liquid):
    ...
    # 기존 global baseline 기준 라벨을 유지하되 local baseline 변화도 같이 반영
    local_change = np.max(np.abs(sig - local_ref))
    label = liquid if (np.max(global_chg) >= CHANGE_THRESHOLD or local_change >= LOCAL_CHANGE_THRESHOLD) else '바닥'
    return window, label
    ...
    for floor, file_list in floor_files.items():
        ...
        model.fit(
            ...
            callbacks=[
                ...
                ModelCheckpoint(os.path.join(save_dir, f'model_{floor}.keras'), save_best_only=True, monitor='val_accuracy')
            ],
            verbose=1
        )
