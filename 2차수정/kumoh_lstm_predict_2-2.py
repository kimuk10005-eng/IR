...
FLOOR_THRESHOLDS = {
    '나무': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '황대': {'gradient': 0.6, 'change_low': 5.0, 'change_strong': 15.0, 'plateau_mean': 7.0, 'plateau_med': 6.0, 'clear_mean': 2.3, 'sign_keep': 0.88},
    '회대': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '검대': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '그마': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '207회바': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '흰책상': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '나타': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
    '회타': {'gradient': 0.7, 'change_low': 6.0, 'change_strong': 18.0, 'plateau_mean': 9.0, 'plateau_med': 7.0, 'clear_mean': 2.8, 'sign_keep': 0.88},
}
...
def read_csv_with_fixed_baseline(fp):
    ...
    def parse_old_format(lines):
        ...
        for raw_line in lines:
            ...
            # 예: Collecting baseline...,RawAvg,416.8
            ...
            # 예: Final,Base,Raw,Average,417.68
            ...
            # 예:
            # Time,0.1s,|,BaseRaw,417.7,|,CurrentRaw,418.4,|,Change,0.0,...
            if parts[0] == 'Time':
                time_val = None
                base_raw = None
                current_raw = None
...
def build_recent_floor_reference(signal, baseline):
    ...
    for i in range(1, len(sig)):
        stable_grad = abs(grad[i]) < 0.8
        close_to_ref = abs(sig[i] - ref[i - 1]) < 6.5
...
def apply_conservative_postprocess(predictions, floor_type):
    ...
    while i < n:
        ...
        if run_len <= 2 and max_change_run < params['change_strong'] + 2 and mean_diff_run < params['plateau_mean'] + 1.0:
