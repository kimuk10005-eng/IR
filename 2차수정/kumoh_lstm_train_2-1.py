...
LOW_CHANGE_THRESHOLD = 4.0
PLATEAU_MEAN_THRESHOLD = 6.0
PLATEAU_MEDIAN_THRESHOLD = 5.0
...
def create_window_with_label(signal, changes, floor_ref, start, base, floor_type, liquid):
    ...
    liquid_flag = (
        max_change >= CHANGE_THRESHOLD or
        (
...
def train_models(data_dir=DATA_DIR, save_dir=SAVE_DIR):
    ...
    for floor, file_list in floor_files.items():
        ...
        for fp in file_list:
            ...
            if base is None:
                continue
            smooth_sig, floor_ref = build_recent_floor_reference(signal, base)
