...
LOCAL_MEAN_THRESHOLD = 1.8
LOCAL_AREA_THRESHOLD = 28.0
LOCAL_REF_BACK = 20
LOCAL_REF_GUARD = 3
SMOOTH_KERNEL = 5
...
def create_window_with_label(signal, changes, start, base, floor_type, liquid):
    ...
    label = liquid if (
        np.max(global_chg) >= CHANGE_THRESHOLD
        or local_change >= LOCAL_CHANGE_THRESHOLD
        or mean_local >= LOCAL_MEAN_THRESHOLD
        or area_local >= LOCAL_AREA_THRESHOLD
    ) else '바닥'
