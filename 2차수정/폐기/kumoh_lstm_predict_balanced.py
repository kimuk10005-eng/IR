...
FLOOR_THRESHOLDS = {
    '나무': {
        'gradient': 0.5, 'change': 5, 'local_change': 4.0,
        'start_grad': 2.0, 'start_mean_local': 6.0, 'start_max_local': 12.0, 'start_area_local': 120.0,
        'very_start_mean_local': 18.0,
        'keep_mean_local': 4.0, 'keep_max_local': 8.0, 'keep_grad': 1.0,
        'release_mean_local': 2.0, 'release_max_local': 4.0, 'release_grad': 0.9,
        'memory_windows': 1, 'release_windows': 2, 'force_release_windows': 3,
        'model_liquid_prob': 78.0
    },
    '황대': {
        'gradient': 0.5, 'change': 3, 'local_change': 3.0,
        'start_grad': 2.5, 'start_mean_local': 7.0, 'start_max_local': 14.0, 'start_area_local': 140.0,
        'very_start_mean_local': 20.0,
        'keep_mean_local': 4.5, 'keep_max_local': 9.0, 'keep_grad': 1.2,
        'release_mean_local': 2.5, 'release_max_local': 5.0, 'release_grad': 1.0,
        'memory_windows': 1, 'release_windows': 2, 'force_release_windows': 3,
        'model_liquid_prob': 80.0
    },
    '회대': {
        'gradient': 0.5, 'change': 3, 'local_change': 3.0,
        'start_grad': 3.0, 'start_mean_local': 8.0, 'start_max_local': 16.0, 'start_area_local': 180.0,
        'very_start_mean_local': 24.0,
        'keep_mean_local': 5.0, 'keep_max_local': 10.0, 'keep_grad': 1.2,
        'release_mean_local': 3.0, 'release_max_local': 6.0, 'release_grad': 1.2,
        'memory_windows': 1, 'release_windows': 2, 'force_release_windows': 3,
        'model_liquid_prob': 82.0
    },
    '검대': {
        'gradient': 0.5, 'change': 3, 'local_change': 3.0,
        'start_grad': 2.2, 'start_mean_local': 8.0, 'start_max_local': 18.0, 'start_area_local': 180.0,
        'very_start_mean_local': 26.0,
        'keep_mean_local': 5.5, 'keep_max_local': 12.0, 'keep_grad': 1.2,
        'release_mean_local': 3.0, 'release_max_local': 6.5, 'release_grad': 1.0,
        'memory_windows': 1, 'release_windows': 2, 'force_release_windows': 3,
        'model_liquid_prob': 82.0
    },
    '그마': {
...
def summarize_window_state(r, params):
    ...
    strong_offset = (
        r['mean_local_change'] >= params['start_mean_local']
        and r['max_local_change'] >= params['start_max_local']
    )
    very_strong_offset = (
        r['mean_local_change'] >= params['very_start_mean_local']
        or (
            r['area_local_change'] >= params['start_area_local']
            and r['max_local_change'] >= params['start_max_local']
        )
    )
    keep_offset = (
        r['mean_local_change'] >= params['keep_mean_local']
        and r['max_local_change'] >= params['keep_max_local']
    )
    release_ready = (
        r['mean_local_change'] <= params['release_mean_local']
        and r['max_local_change'] <= params['release_max_local']
        and r['max_gradient'] < params['release_grad']
    )
    model_liquid = (r['orig_label'] != '바닥') and (r['orig_prob'] >= params['model_liquid_prob'])
    model_strong = model_liquid and (r['max_gradient'] >= params['keep_grad']) and (r['max_local_change'] >= params['keep_max_local'])
    return strong_boundary, strong_offset, very_strong_offset, keep_offset, release_ready, model_liquid, model_strong
...
def apply_hysteresis_postprocess(predictions, floor_type):
    ...
    for r in predictions:
        strong_boundary, strong_offset, very_strong_offset, keep_offset, release_ready, model_liquid, model_strong = summarize_window_state(r, params)

        start_trigger = (
            (strong_boundary and strong_offset)
            or very_strong_offset
            or model_strong
        )

        keep_trigger = (
            keep_offset and (memory > 0 or r['max_gradient'] >= params['keep_grad'])
        ) or model_strong
        ...
        if state == 0:
            ...
            if keep_trigger:
                ...
                release_count = 0
                weak_count += 1
                if weak_count >= params['force_release_windows']:
                    state = 0
                    memory = 0
                    weak_count = 0
    ...
    for i in range(1, len(labels) - 1):
        if labels[i] == 0 and labels[i - 1] == 1 and labels[i + 1] == 1:
            bridge_ok = (
                predictions[i]['mean_local_change'] >= params['keep_mean_local'] * 0.9
                and predictions[i]['max_local_change'] >= params['keep_max_local'] * 0.9
            )
            if bridge_ok:
                labels[i] = 1
    ...
    for i, lab in enumerate(labels):
        if lab == 0:
            ...
            if predictions[i]['label'] == '바닥' or predictions[i]['prob'] < 78.0:
...
def predict(buffer, global_changes, baseline, floor_type, model):
    ...
    for start_idx in range(0, max_start + 1, STEP):
        ...
        strong_global = (max_global_change >= max(45.0, global_change_thresh * 10.0)) or (avg_global_change >= 30.0)
        ...
        event_score = (
            ...
            + max_local_change / (params['start_max_local'] + 1e-8)
        )

        if strong_global and orig_label != '바닥' and orig_prob >= params['model_liquid_prob']:
            final_label = orig_label
            final_prob = max(orig_prob, 86.0)
