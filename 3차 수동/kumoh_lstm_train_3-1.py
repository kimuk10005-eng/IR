# -*- coding: utf-8 -*-
"""
기존 2-4 구조 유지 + 평탄 구간 억제 강화 버전 (학습)

핵심
- 기존 4채널 유지: norm, deriv1, deriv2, direction_channel
- 기존 WINDOW 기반 유지
- manual split(_rise/_fall) 파일도 학습 가능
- 평탄 구간은 바닥으로 더 강하게 넣고
- 액체는 "기울기 이벤트 구간" 위주로만 학습

권장 데이터 구성
1) 일반 시나리오 CSV
2) 수동으로 자른 _rise / _fall CSV
둘 다 같은 폴더에 두고 학습 가능
"""
...
# 설정
# ===========================
DATA_DIR = r"C:\Users\MASL\Desktop\데이터 크롭\예시"
...
# 평탄 억제용 추가 기준
EDGE_GRAD_THRESHOLD = 0.85         # rise/fall로 볼 최소 기울기
EDGE_RATIO_THRESHOLD = 0.28        # 윈도우 내 edge 비율
FLAT_STD_THRESHOLD = 0.75          # 평탄 구간 표준편차
FLAT_GRAD_THRESHOLD = 0.45         # 평탄 구간 최대 기울기
PLATEAU_KEEP_PROB = 0.15           # plateau 액체 윈도우는 일부만 유지
...
def classify_window_for_training(sig_w, chg_w, diff_w, grad_w, floor_type, liquid, is_manual_event=False):
    ...
    if is_manual_event:
        # 수동으로 잘린 rise/fall은 액체 이벤트 중심 데이터이므로,
        # plateau는 거의 버리고 edge 위주만 액체로 사용
        if clear_edge_event and max_change >= LOW_CHANGE_THRESHOLD:
            return liquid
        if plateau_liquid and random.random() < PLATEAU_KEEP_PROB:
            return liquid
        return None
...
def train_models(data_dir=DATA_DIR, save_dir=SAVE_DIR):
    ...
    for floor, file_list in floor_files.items():
        ...
        for fp in file_list:
            ...
            # 바닥 전용 파일은 따로 없더라도 일반 시나리오에서 바닥 윈도우가 생성됨
            ...
            for start in range(0, max_start + 1, step):
                win = create_window(smooth_sig, start, base, floor)
                if win is None:
                    continue
