...
def infer_phase(avg_change, max_change, mean_floor_diff, max_gradient, params):
    if max_change < params['change_low'] * 0.9 and mean_floor_diff < params['clear_mean']:
        return 'floor_return'
    if mean_floor_diff >= params['plateau_mean'] and avg_change >= params['change_low']:
        return 'plateau'
    if max_gradient >= params['gradient']:
        return 'rise/fall'
    return 'mixed'
...
def predict(buffer, changes, baseline, floor_type, model, classes):
    ...
    for start_idx in range(0, max_start + 1, STEP):
        ...
        hard_floor = max_change < params['change_low'] * 0.90 and mean_floor_diff < params['clear_mean'] and max_gradient < params['gradient'] * 0.90
        plateau_flag = mean_floor_diff >= params['plateau_mean'] and median_floor_diff >= params['plateau_med'] and sign_consistency >= params['sign_keep'] and noise_ratio <= params.get('noise_ratio', 0.8) and spike_ratio <= params.get('spike_ratio', 2.0)
        very_clear_plateau = mean_floor_diff >= params['plateau_mean'] + 1.8 and median_floor_diff >= params['plateau_med'] + 1.2 and sign_consistency >= 0.92 and noise_ratio <= params.get('noise_ratio', 0.8) * 0.9 and spike_ratio <= params.get('spike_ratio', 2.0) * 0.9
        ...
        phase_hint = infer_phase(avg_change, max_change, mean_floor_diff, max_gradient, params)
        predictions.append({
            'start': start_idx,
...
def visualize(predictions, buffer, changes, baseline, floor_type):
    ...
    gs = fig.add_gridspec(5, 1, height_ratios=[3.0, 2.2, 1.2, 1.3, 1.8], hspace=0.30)
    fig.suptitle(f'{floor_type} 바닥 - 4분할 학습 모델 결과', fontsize=20, fontweight='bold', y=0.97)
    ...
    hint_map = {'floor_return': 0, 'rise/fall': 1, 'plateau': 2, 'mixed': 3}
    hint_vals = [hint_map.get(r['phase_hint'], 3) for r in predictions]
    ax4.plot(hint_vals, marker='o', linewidth=1.3)
    ax4.set_yticks([0, 1, 2, 3])
    ax4.set_yticklabels(['floor_return', 'rise/fall', 'plateau', 'mixed'])
    ax4.set_title('구간 힌트(참고용)')
    ax4.grid(alpha=0.25)

    ax5 = fig.add_subplot(gs[4])
    for i, r in enumerate(predictions):
        color = FLOOR_COLOR if r['label'] == '바닥' else LIQUID_COLOR
        ax5.add_patch(Rectangle((i, 0), 1, 1, facecolor=color, edgecolor='k', lw=1.2))
        txt = '바' if r['label'] == '바닥' else '액'
        ax5.text(i + 0.5, 0.5, txt, ha='center', va='center', fontweight='bold', fontsize=10 if len(predictions) < 50 else 8, color='white')
    ax5.set_xlim(0, len(predictions))
    ax5.set_ylim(0, 1)
    ax5.set_yticks([])
    ax5.set_title('예측 시퀀스')
    legend_handles = [
        Patch(facecolor=FLOOR_COLOR, edgecolor='black', label='바닥'),
        Patch(facecolor=LIQUID_COLOR, edgecolor='black', label='액체')
    ]
    ax5.legend(handles=legend_handles, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2, fontsize=12)
...
def main():
    print('\n' + '=' * 80)
    print('LSTM 액체 감지 예측기 - 4분할 학습 대응 버전'.center(80))
    print('=' * 80 + '\n')
    ...
    while True:
        ...
        try:
            ...
            phase_counts = Counter([p['phase_hint'] for p in preds])
