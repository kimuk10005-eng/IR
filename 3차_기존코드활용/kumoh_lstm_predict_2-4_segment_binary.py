...
WINDOW = 15
STEP = 3  # 짧은 액체구간을 더 잘 잡기 위해 보폭만 조금 촘촘하게
SMOOTH_K = 5
...
def get_liquid_prob(pred):
    if '액체' in classes:
        idx = np.where(classes == '액체')[0][0]
        return float(pred[idx] * 100.0)
    floor_idx = np.where(classes == '바닥')[0][0] if '바닥' in classes else 0
    return float((1.0 - pred[floor_idx]) * 100.0)


def bridge_and_clean(predictions):
    if not predictions:
        return predictions
    ...
    for i in range(1, n - 1):
        if labels[i] == '액체' and labels[i - 1] == '바닥' and labels[i + 1] == '바닥':
            ...
            predictions[i]['prob'] = max(predictions[i]['prob'], 70.0)

    labels = [p['label'] for p in predictions]
    # 짧은 바닥 틈은 액체로 연결
    for i in range(1, n - 1):
        if labels[i] == '바닥' and labels[i - 1] == '액체' and labels[i + 1] == '액체':
            left = predictions[i - 1]
            mid = predictions[i]
            right = predictions[i + 1]
            if mid['max_change'] >= min(left['max_change'], right['max_change']) * 0.55 or mid['mean_floor_diff'] >= min(left['mean_floor_diff'], right['mean_floor_diff']) * 0.7:
                predictions[i]['label'] = '액체'
                predictions[i]['prob'] = max(68.0, mid['liquid_prob'])
...
def predict(buffer, changes, baseline, floor_type, model):
    ...
    for start_idx in range(0, max_start + 1, STEP):
        ...
        pred = model.predict(np.transpose(window, (0, 2, 1)), verbose=0)[0] if model else np.array([1.0, 0.0], dtype=np.float32)
        liquid_prob = get_liquid_prob(pred)
        ...
        if hard_floor:
            final_label = '바닥'
            final_prob = max(75.0, 100.0 - liquid_prob)
        elif strong_liquid and liquid_prob >= 52.0:
            final_label = '액체'
            final_prob = max(82.0, liquid_prob)
        elif simple_liquid and plateau_flag and liquid_prob >= 45.0:
            final_label = '액체'
            final_prob = max(72.0, liquid_prob)
        elif simple_liquid and plateau_flag:
            final_label = '액체'
            final_prob = max(66.0, liquid_prob)
        elif liquid_prob >= 72.0 and max_change >= params['change_low'] * 0.75:
            final_label = '액체'
            final_prob = liquid_prob
        else:
            final_label = '바닥'
            final_prob = max(60.0, 100.0 - liquid_prob)
    ...
    return bridge_and_clean(predictions)
...
def main():
    print('\n' + '=' * 80)
    print('LSTM 액체 감지 예측기 - segment binary refined'.center(80))
    print('=' * 80 + '\n')
