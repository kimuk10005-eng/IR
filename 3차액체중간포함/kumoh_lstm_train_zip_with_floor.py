...
from tensorflow.keras.layers import (LSTM, BatchNormalization, Bidirectional, Conv1D,
...
# liquid zip 안의 head/tail floor 조각을 최대한 걸러내기 위한 조건
LIQUID_POINT_THRESHOLD = 5.0
LIQUID_WINDOW_ACTIVE_RATIO = 0.40
MIN_STEP_WINDOWS = 25
...
def should_keep_liquid_window(chg_window: np.ndarray) -> bool:
    active_ratio = float(np.mean(chg_window >= LIQUID_POINT_THRESHOLD))
    center = chg_window[WINDOW_SIZE // 3: WINDOW_SIZE - WINDOW_SIZE // 3]
    center_active = float(np.max(center)) >= LIQUID_POINT_THRESHOLD if len(center) else False
    return active_ratio >= LIQUID_WINDOW_ACTIVE_RATIO or center_active
...
def build_dataset_for_floor(file_list, floor_type):
    ...
    for fp in sorted(file_list):
        ...
        segment_type = 'floor' if is_floor_segment_file(fp) else 'liquid'
        ...
        for start in range(0, max_start + 1, step):
            ...
            if segment_type == 'floor':
                ...
                if not should_keep_liquid_window(chg_w):
                    continue
                label = liquid_label
